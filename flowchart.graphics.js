...
(function(root, factory) {
	...
	Graphics.Api = {
		...
		debounce: function(delay, fn) {
			...
			return function(){
				...
				timer = setTimeout(function(){
					void(timer = null, fn.apply(that, args));
				}, delay);
			};
		},
		...
		stringify: function(object, indentMode){
			var dejaVu = [], sep, delim = ',', pretty = jQuery.noop, serialize = function(data, buffer, path, indentLevel){
				...
				case '[object Array]':
					dejaVu.push(data);
					buffer.push('[');
					pretty(buffer, true, 0);
					sep = '';
					for (var key in data) {
						buffer.push(sep);
						pretty(buffer, false, indentLevel);
						serialize(data[key], buffer, path + '[' + key + ']', indentLevel + 1);
						sep = delim;
					}
					pretty(buffer, !!sep, --indentLevel);
					buffer.push(']');
					break;
				case '[object Object]':
					dejaVu.push(data);
					...
					break;
				case '[object String]':
					buffer.push('"', Graphics.Api.quote(data), '"');
					break;
				case '[object Number]':
					buffer.push(data);
					break;
				...
				case '[object Null]':
					...
					break;
				case '[object Date]':
					buffer.push('"', data.getFullYear(), '-', data.getMonth() + 1, '-', data.getDate(), ' ', data.getHours(), ':', data.getMinutes(), ':', data.getSeconds(), '"');
					break;
				case '[object Boolean]':
					buffer.push(data ? 'true' : 'false');
					break;
				default:
					buffer.push('"', Graphics.Api.quote(String(data)), '"');
					break;
				}
				return buffer;
			};
			if (indentMode === true) {
				void(delim += '\n', pretty = function(buffer, addLine, indentLevel){
					if (addLine === true) {
						buffer.push('\n');
					}
			...
			return serialize(object, [], '#root', 1).join('');
		}
	};
	...
	if (Graphics.VML.supported){
		...
		Graphics.extend(Graphics.Painter = Graphics.VML, {
			...
			Text: Graphics.create({
				create: function(leading, aligning){
					this._left = 4;
					this._top = 0;
				...
				superclass: Graphics.Container,
				constructors: {
					text: function(text, leading, aligning, background){
						return this.put(new Graphics.VML.Text(leading, aligning)).css({ border: 'none', 'background': background === true ? '#f8f9fc' : 'transparent', position: 'absolute', display: 'block', cursor: 'text' }).text(text);
					}
				},
				methods: {
					text: function(text){
					...
					updateBG: function(bgColor, borderColor){
						return this.css({ border: borderColor || this.css('border'), 'background': bgColor || this.css('background-color'), padding: this._top + 'px ' + this._left + 'px' });
					},
					updateUI: function(){
						if (this.dom && this.text()) {
							...
							if (!(parent instanceof Graphics.DOMElement)) {
								...
								if (x || y) {
									return this.css({ left: x - this._left / 2 + 'px', top: y - this._top / 2 + 'px' }).updateBG('white');
								}
							}
		...
		Graphics.extend(Graphics.Painter = Graphics.SVG, {
			...
			Filter: Graphics.create({
				create: 'filter',
				superclass: Graphics.Container,
				constructors: {
					filter: function(type) {
						return this.put(new Graphics.SVG.Filter);
					}
				}
			}),
			Rect: Graphics.create({
			...
			Text: Graphics.create({
				create: function(leading, aligning, bgRect){
					this._bgRect = bgRect;
				...
				constructors: {
					text: function(text, leading, aligning, background){
						var bgRect = background === true ? this.put(new Graphics.SVG.Rect).rx(0).ry(0).css({ fill: '#f8f9fc', stroke: 'none', 'stroke-dasharray': '5,5', 'stroke-width': '1px' }) : null;
						return this.put(new Graphics.SVG.Text(leading, aligning, bgRect)).css({ fill: 'black', stroke: 'none', 'stroke-width': 1, 'font-size': Graphics.Config.defaultAttrs['font-size'], 'font-family': Graphics.Config.defaultAttrs['font-family'], cursor: 'text' }).text(text);
					}
				},
				methods: {
					text: function(text){
					...
					updateBG: function(bgColor, borderColor){
						if (this._bgRect) {
							var bbox = this.bbox();
							this._bgRect.size(bbox.width + 8, bbox.height + 4).move(bbox.x - 4, bbox.y - 2).css({ fill: bgColor || this._bgRect.css('fill'), stroke: borderColor || this._bgRect.css('stroke') });
						}
						return this;
					},
					updateUI: function(){
						if (this.dom && this.text()) {
							...
							if (!(parent instanceof Graphics.DOMElement) && parent.x() === 0) {
								...
								if (x || y) {
									...
									return this.css('text-anchor', 'middle').attr('x', x).y(y).updateBG();
								}
							}
		...
		Graphics.extend(Graphics.Container, {
			namespace: function(){
				this.attr('version', '1.1').attr('xmlns:xlink', Graphics.SVG.xlink, Graphics.SVG.xmlns);
				/* FIX MSIE */
